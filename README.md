...
Put it somewhere writable — Desktop or Downloads, not Program Files. It keeps
its data in a folder beside itself, and replaces itself in place when it
updates.
...
Both trade directions are lopsided in the same way, and that shapes every
column:
...
rate instead — retainer listing slots, not gil, are what you run out of, and one
slot holds one listing of up to a full stack. The two disagree when stock is
thin, and trip value is usually the honest one.
...
data-centre median sale, margins over 1500% (private trades, not demand), items
with fewer than four recorded sales, and anything that hasn't sold in a week.
The buy floor also ignores listings under a quarter of the median, so one person
dumping a unit far below market can't delete an item from the tables.
...
| **Look up an item** | price one item everywhere, unfiltered, in seconds |
...
Three hosts, HTTPS only, and it refuses any other URL or redirect:
...
Estimates from crowd-sourced data, not guarantees. Prices move, a row backed by
four sales is a guess, and the figures assume you hold the lowest price long
enough to capture that world's sales. If something looks wrong, `error.log` in
the data folder records crashes with the version number.
